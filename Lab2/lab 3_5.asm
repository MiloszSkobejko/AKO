...
; ### FUNKCJA WYSWIETLANIA LICZBY HEXALNIE W KONSOLI
wyswietl_EAX_hex PROC
			...
			; rezerwacja 12 bajtów na stosie (poprzez zmniejszenie; rejestru ESP) przeznaczonych na tymczasowe przechowanie; cyfr szesnastkowych wyświetlanej liczby
				sub		esp, 12
				mov		edi, esp								; adres zarezerwowanego obszaru pamięciprzygotowanie konwersji
			...
			; Usuniecie niepotrzebnych zer z poczatku
				mov		ecx, 8									; liczba obiegów pętli zamiany zer, ustawiona na pierwszy bit
				mov		esi, 1									; indeks początkowy używany przy ; zapisie cyfr; pętla konwersji

				ptlspace:
						mov		dl,  30H						; 30 to kod zera ASCII hexalnie
						cmp		[edi][esi], dl
						jne		done

						mov		byte PTR [edi][esi], 20H		; dodawanie spacji w miejsce zera
						inc		esi
						loop	ptlspace
				done:
